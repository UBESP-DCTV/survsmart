############iniziofunzione---
LDTestimate_gl<-function (data, L = .Machine$double.xmax) 
{
  if (is.null(data$X)) 
  ...
  t <- t[order(t)]
  n.risk <- apply(as.array(t), 1, function(x) sum(as.numeric(data$U >= 
                                                               x)))
  n.event <- apply(as.array(t), 1, function(x) length(which(data$U == 
                                                              x & data$delta == 1)))
  cat("Estimating for A1 arm... \n")
  est1 <- sub.LDTestimate(pdata = data[which(data$X == 0), 
  ], t, L)
  cat("Estimating for A2 arm... \n")
  est2 <- sub.LDTestimate(pdata = data[which(data$X == 1), 
  ], t, L)
  cat("Estimating for A3 arm... \n")
  est3 <- sub.LDTestimate(pdata = data[which(data$X == 2), 
  ], t, L)
  results <- list(Call = match.call(), DTR = c("A1B1", "A1B2", "A2B1", "A2B2", "A3B1", "A3B2"), 
                  records = c(
                  ...
                  censorsurv = c(
                    apply(as.array(data$U[which((data$X == 
                                                   0 & data$R == 1 & data$Z == 0 & data$delta == 0) | 
                                                  (data$X == 0 & data$R == 0 & data$delta == 0))]), 
                          1, function(x) {
                            if (x < min(est1$t)) 1 else est1$SURV1[which(abs(est1$t - 
                    ...
                    apply(as.array(data$U[which((data$X == 0 & data$R == 
                                                  ...
                                                  if (x < min(est1$t)) 1 else est1$SURV2[which(abs(est1$t - 
                    ...
                    apply(as.array(data$U[which((data$X == 1 & data$R == 
                                                  ...
                                                  if (x < min(est2$t)) 1 else est2$SURV1[which(abs(est2$t - 
                                                  x) == min(abs(est2$t - x)[which(est2$t <= x)]))]
                                                                                           }), 
                    apply(as.array(data$U[which((data$X == 1 & data$R == 
                                                   1 & data$Z == 1 & data$delta == 0) | (data$X == 
                                                   1 & data$R == 0 & data$delta == 0))]), 1, function(x) {
                                                  if (x < min(est2$t)) 1 else est2$SURV2[which(abs(est2$t - 
                                                  x) == min(abs(est2$t - x)[which(est2$t <= x)]))]
                                                                                           }),
                    apply(as.array(data$U[which((data$X == 2 & data$R == 
                                                   1 & data$Z == 0 & data$delta == 0) | (data$X == 
                                                   2 & data$R == 0 & data$delta == 0))]), 1, function(x) {
                                                  if (x < min(est3$t)) 1 else est3$SURV1[which(abs(est3$t - 
                                                  x) == min(abs(est3$t - x)[which(est3$t <= x)]))]
                                                                                           }), 
                    apply(as.array(data$U[which((data$X == 2 & data$R == 
                                                   1 & data$Z == 1 & data$delta == 0) | (data$X == 
                                                   2 & data$R == 0 & data$delta == 0))]), 1, function(x) {
                                                   if (x < min(est3$t)) 1 else est3$SURV2[which(abs(est3$t - 
                                                   x) == min(abs(est3$t - x)[which(est3$t <= x)]))]
                                                                                           })
                  ), 
