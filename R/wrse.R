wrse <- function(db, L = .Machine$double.xmax) {
  ...
  WRSEestimate_gl <- function(db) {
    ...
    est1 <- sub.WRSEestimate(pdata = db[which(db$X == 0), ], t)
    cat("Estimating for A2 arm... \n")
    est2 <- sub.WRSEestimate(pdata = db[which(db$X == 1), ], t)
    cat("Estimating for A3 arm... \n")
    est3 <- sub.WRSEestimate(pdata = db[which(db$X == 2), ], t)
